...
if ( class_exists( 'Lablab_Module_Builder' ) ):
	...
	class Lablab_Builder_Slideshow_Module extends Lablab_Module_Builder {
		...
		public function __construct(){
			...
			// The module title.
			$this->title = __( 'Slideshow', 'lablab-slideshow' );
			...
			$this->template_path = plugin_dir_path( __DIR__ ) . 'template/lablab-slideshow.php';

			// The javascript file(s) to be included only on admin pages.
			$this->admin_js = plugin_dir_url( __DIR__ ) . 'js/lablab-slideshow-admin.js';

			// The css file(s) to be included only on admin pages.
			$this->admin_css = plugin_dir_url( __DIR__ ) . 'css/lablab-slideshow-admin.css';
